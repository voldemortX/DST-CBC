    ...
    if label_ratio >= 1:
        ...
        with torch.no_grad():
            for images, _ in tqdm(loader):
                ...
                # Count
                probabilities = np.concatenate((probabilities, pseudo_probabilities.cpu().numpy()))

        # Sort (n * log(n) << n * label_ratio, so just sort is good) and find kc
        print('Sorting...')
        probabilities.sort()
        kc = [probabilities[-int(len(probabilities) * label_ratio) - 1] for _ in range(num_classes)]
