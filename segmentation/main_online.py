    ...
    if state == 3:
        ...
        # Semi-supervised training
            ...
            unlabeled_set = StandardSegmentationDataset(root=base, data_set=data_set, mask_type='.npy',
                                                        ...
                                                        transforms=transform_train_pseudo, label_state=2)
            unlabeled_loader = torch.utils.data.DataLoader(dataset=unlabeled_set, batch_size=batch_size_pseudo,
                                                           num_workers=workers, shuffle=True)
...
def train(writer, loader_c, loader_sup, validation_loader, device, criterion, net, optimizer, lr_scheduler,
    ...
    while epoch < num_epochs:
        ...
        for i, data in enumerate(loader_c, 0):
            ...
            labels_placeholder = torch.ones(inputs_c.shape[0], inputs_c.shape[2], inputs_c.shape[3]).long()
            labels_sup = torch.cat([labels_placeholder, labels_sup], dim=0)
