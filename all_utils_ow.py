    ...
    if state == 3:
        ...
        # Semi-supervised training
            ...
            reference_set = SegmentationLabelsDataset(root=base, image_set=(str(split) + '_unlabeled_' + str(sets_id)),
                                                      data_set=data_set, mask_type='.npy')
    ...
    while epoch < num_epochs:
        ...
        for i, data in enumerate(loader_c, 0):
            ...
            if with_sup:
                ...
                p = labels_c[:, 1, :, :].clone()
                labels_c = torch.cat([labels_c[:, 0, :, :].long(), labels_sup])
                labels = labels_c
            else:
                inputs, labels = data
            ...
            if with_sup:
                loss, statistics = criterion(outputs, labels, p, inputs_c.shape[0])
            else:
...
def generate_pseudo_labels(net, device, loader, num_classes, input_size, cbst_thresholds=None):
    ...
    with torch.no_grad():
        for images, file_name_lists in tqdm(loader):
            ...
            p = permuted.clone().detach().max(dim=1).values.unsqueeze(-1)
            ...
            pseudo_labels = pseudo_labels.unsqueeze(-1).float()
            pseudo_labels = torch.cat([pseudo_labels, p], dim=-1)

            # Counting & Saving
            labeled_counts += (pseudo_labels[:, :, :, 0] != 255).sum().item()
            ignored_counts += (pseudo_labels[:, :, :, 0] == 255).sum().item()
